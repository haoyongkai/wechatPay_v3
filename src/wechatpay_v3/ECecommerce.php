...
class  ECecommerce{
   ...
   */
     ...
     * 接口规则：https://wechatpay-api.gitbook.io/wechatpay-api-v3
     */
    public function fundEnddaybalance($sub_mchid,$date){
        // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
        $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
            'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
        $url = 'https://api.mch.weixin.qq.com/v3/ecommerce/fund/enddaybalance/'.$sub_mchid.'?date='.$date;
        try {
            $resp = $this->client->request('GET', $url,
                ['headers' =>$headers]);
            return $resp->getBody()->getContents();
     ...
     * 接口规则：https://wechatpay-api.gitbook.io/wechatpay-api-v3
     */
    public function fundBalanceAccount($account_type){
        // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
        $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
            'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
     ...
     * 接口规则：https://wechatpay-api.gitbook.io/wechatpay-api-v3
     */
    public function fundDayendbalance($account_type,$date){
        // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
        $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
            'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
        $url = 'https://api.mch.weixin.qq.com/v3/merchant/fund/dayendbalance/'.$account_type.'?date='.$date;
        try {
            $resp = $this->client->request('GET', $url,
                ['headers' =>$headers]);
            return $resp->getBody()->getContents();
