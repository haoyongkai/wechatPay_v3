...
class  ECfund{
   ...
   */
     ...
     */
   public function fundWithdraw($form_params){
       // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
       $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
           'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
       try {
           $resp = $this->client->request('POST', 'https://api.mch.weixin.qq.com/v3/ecommerce/fund/withdraw',
    ...
    *接口规则：https://wechatpay-api.gitbook.io/wechatpay-api-v3
     */
    public function fundWithdrawId($sub_mchid,$withdraw_id){
        // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
        $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
            'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
    ...
    *接口规则：https://wechatpay-api.gitbook.io/wechatpay-api-v3
     */
    public function fundWithdrawBill($bill_type,$bill_date,$tar_type){
        // 接下来，正常使用Guzzle发起API请求，WechatPayMiddleware会自动地处理签名和验签
        $headers = ['Content-type' => 'application/json;','Accept' => 'application/json',
            'User-Agent' => $_SERVER['HTTP_USER_AGENT']];
        $url = 'https://api.mch.weixin.qq.com/v3/merchant/fund/withdraw/bill-type/'.$bill_type.'?bill_date='.$bill_date.'&tar_type='.$tar_type;
        try {
            $resp = $this->client->request('GET', $url,
                ['headers' =>$headers]);
            return $resp->getBody()->getContents();
